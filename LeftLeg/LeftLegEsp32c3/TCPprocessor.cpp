...
void TCPprocessor::begin() {
        ...
        */
        ...
        const char* ntpServer = "192.168.66.1";   
        const long  gmtOffset = 3600;               // seconds; use +3600 for UTC+1, etc.
        const int   dstOffset = 3600;            // daylight-saving offset, or 0

        configTime(gmtOffset, dstOffset, ntpServer);  // ask router for SNTP
        struct tm tmNow;
        if (getLocalTime(&tmNow, 10000)) {       // wait up to 10 s
            Serial.println(&tmNow, "%F %T");     // 2025-08-01 12:34:56
        } else {
