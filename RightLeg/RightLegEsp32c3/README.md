...
# RightLegEsp32c3 – micro-ROS radar + taster

Firmware for a **Seeed XIAO ESP32-C3** in the **right leg**.
...
- `/leg_r/taster_event` (`leg_msg/msg/TasterEvent`)
- `/leg_r/radar` (`leg_msg/msg/Ld2410State`)
...
```bash
ros2 topic list | grep leg_r
```

Echo button events:

```bash
ros2 topic echo /leg_r/taster_event
```

Echo radar data:

```bash
ros2 topic echo /leg_r/radar
```
