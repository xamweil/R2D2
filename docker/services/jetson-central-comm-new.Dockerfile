...
RUN mkdir -p /tmp/just && \
    curl -L https://github.com/casey/just/releases/download/1.42.4/just-1.42.4-x86_64-unknown-linux-musl.tar.gz | tar -xz -C /tmp/just && \
...
COPY $ROOT/Body/BodyJetson/central_comm/ $OVERLAY_WS/

RUN rosdep update --rosdistro $ROS_DISTRO
RUN rosdep install -i --from-path src --rosdistro $ROS_DISTRO -y

RUN . /opt/ros/$ROS_DISTRO/setup.sh && \
    colcon build --symlink-install && rm -rf src build log
