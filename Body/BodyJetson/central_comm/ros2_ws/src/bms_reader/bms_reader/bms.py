...
class BMS:
    ...
    def _ordered_values(self, mapping: Optional[Dict[int, float]]) -> List[float]:
            if not mapping:
                return []
            # mapping is {index: value}, ROS needs a list in order 1..N
            return [mapping[k] for k in sorted(mapping.keys())]

    def _map_status(self, mosfet_status: Optional[dict], soc_pct: Optional[float]) -> int:
        """
